...
public class ChooseWebdriverFactory implements IBasicCommand {
	private static final Logger logger = LogManager.getLogger( ChooseWebdriverFactory.class );
	...
	public void run( IProduct prod, IEnvironment env, Object extra, int index ) throws Exception {
		...
		try {
			if ( product.isUsingLogin() || product.isSkipLogin() ) {
				...
				ourEnv.setWebDriverFactory( ourEnv.getWebDriverFactoryFactory().makeWebDriverFactory( WebDriverFactoryType.ORIGINAL_WITH_JAVASCRIPT ) );
			}
		}
		catch ( final Exception e ) {
			ourEnv.logWarn( logger, "error getting testing_account_name or creating webDriverFactory", e );
			throw e;
		}
	}
}
