...
public class CreateWebdriver implements IBasicCommand {
	private static final Logger logger = LogManager.getLogger( CreateWebdriver.class );
	...
	public void run( IProduct product, IEnvironment env, Object extra, int index ) throws Exception {
		...
		try {
			ourEnv.setWebDriver( ourEnv.getWebDriverFactory().makeWebDriver( ourEnv.getBrowserProxy() ) );
