...
public class OverridePreferencesFromEmbedPathsWindows implements IOverridePreferences {
	...
	public OverridePreferencesFromEmbedPathsWindows( final IAppDirectories appDirectories ) {
	...
	public boolean override( final IPreferences prefs, final IResourceBundleWithFormatting bundle ) {
		...
		final File firefoxProfilesDataDir = new File( firefoxDir, prefs.getValue( "embed.firefox.windows.profile_data_dir" ) );
		final File firefoxProfilesDir = new File( firefoxProfilesDataDir, prefs.getValue( "embed.firefox.windows.profile_dir" ) );
		...
		final StringList stringList = new StringList( prefs.getValue( "embed.firefox.windows.binary_names" ) );
		final List<String> binaryNames = stringList.getItems();
		for ( final String binaryName : binaryNames ) {
			final File binaryFile = new File( firefoxDir, binaryName );
