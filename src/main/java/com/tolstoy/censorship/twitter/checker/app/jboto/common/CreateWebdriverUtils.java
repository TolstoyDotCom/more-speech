...
public class CreateWebdriverUtils implements IBasicCommand {
	private static final Logger logger = LogManager.getLogger( CreateWebdriverUtils.class );
	...
	public void run( IProduct product, IEnvironment env, Object extra, int index ) throws Exception {
		...
		try {
			ourEnv.setWebDriverUtils( ourEnv.getWebDriverFactory().makeWebDriverUtils( ourEnv.getWebDriver() ) );
		}
